...
simple_weights = zeros(Weight, num_features(test_features) )
for i=1:(length(simple_weights) / length(all_tags))
     simple_weights[i+length(simple_weights) / length(all_tags)] = 1
end
predicted = predict_label(simple_weights, test_features, test_sentence, all_tags)
